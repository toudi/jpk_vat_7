...
func CipherInit(keysize int) (*Cipher, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Nie udało się wygenerować klucza szyfrującego: %v", err)
	}

	if _, err = rand.Read(_cipher.IV); err != nil {
		return nil, fmt.Errorf("Nie udało się odczytać wektora inicjalizującego: %v", err)
	}

	_cipher.block, err = aes.NewCipher(_cipher.Key)
	if err != nil {
		return nil, fmt.Errorf("Nie udało się stworzyć instancji szyfru AES: %v", err)
	}
