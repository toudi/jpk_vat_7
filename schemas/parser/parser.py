...
def schema_nodes(schema: xmlschema.XMLSchema):
    ...
    def walk_element(xsd_element, path):
        ...
        for child in model.iter_elements():
            ...
            all_types[f"{path}.{child.local_name}"] = child.type.local_name
    ...
    return nodes, required, array_nodes, all_types
...
if __name__ == "__main__":
    ...
    for schema_file in listdir(schemas_dir):
        ...
        package_name = variable_prefix.lower().removeprefix("jpk_")
        target_dir = TARGET_DIR + "/" + package_name
        ...
        with open(target_filename, "w") as ordering_file:
            ...
            field_order, required_fields, array_nodes, all_types = schema_nodes(
            ...
            for node_name, node_type in all_types.items():
                if node_type in REQUIRED_FIELDS["date-types"]:
                    detected_date_fields.append(node_name)
                    continue
            ...
            if len(required_fields) > 0:
                ...
                for node_name, node_type in required_fields.items():
                    ...
                    if not any(
                        ...
                        for prefix in REQUIRED_FIELDS["prefixes"]
                    ):
                        continue
            ...
            if len(detected_date_fields) > 0:
                ordering_file.write(
                    f"var {variable_prefix}DateElements = map[string]bool{{\n"
                )
                for node in detected_date_fields:
                    ordering_file.write(f'"{node}": true,\n')
                ordering_file.write("}")
